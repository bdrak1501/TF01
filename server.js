...
app.post("/create-skup-order", async (req, res) => {
    try {
        ...
        // Automatyczna wysyłka potwierdzenia do klienta
        if (email) {
            sendEmail(
                email, 
                "Zgłoszenie skupu zostało przyjęte – TeleFix Gliwice", 
                `Cześć ${name}!\n\nOtrzymaliśmy Twoje zgłoszenie wyceny i chęci sprzedaży telefonu: ${deviceModel}.\nSzacowana kwota do wypłaty: ${estimatedPrice} zł.\nWybrana forma odbioru: ${method}.\n\nSkontaktujemy się z Tobą telefonicznie lub mailowo w celu sfinalizowania transakcji.\n\nPozdrawiamy,\nZespół TeleFix Gliwice`
            ).catch(e => console.log("Błąd wysyłki e-mail skupu:", e.message));
        }

        res.json({ success: true });
    } catch (err) {
        console.error("Błąd zapisu zamówienia skupu:", err);
